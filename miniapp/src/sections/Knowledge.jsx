...
import { handleHtmlLinkClick } from '../lib/telegram.js';
...
function Article({ article, onOpenSub, onOpenPhoto }) {
  return (
    <div>
      ...
      <div className="card tg-text" onClick={handleHtmlLinkClick} dangerouslySetInnerHTML={{ __html: tgHtml(article.content) }} />
...
export default function Knowledge({ goBack }) {
  ...
  if (sub) {
    return (
      <div>
        ...
        <div className="card tg-text" onClick={handleHtmlLinkClick} dangerouslySetInnerHTML={{ __html: tgHtml(sub.content) }} />
        {lightbox}
      </div>
    );
  }
  ...
  return (
    <div>
      <button className="kb-back" onClick={goBack}><ArrowLeft size={16} /> Назад</button>
