...
const SEED_DATA = {
  screens: {
    ...
    anketa_traffic_category: {
      ...
      buttons: {
        _order: [
          'btn_cpa_net', 'btn_seo', 'btn_aso', 'btn_sms', 'btn_cpa',
          'btn_push', 'btn_inapp', 'btn_dsp', 'btn_social', 'btn_influence', 'btn_other_traffic',
        ],
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (dbId && data && data.screens) {
      ...
      // Реверсная миграция: разбиваем агрегированные «SEO ASO SMS» и
      // «PUSH IN APP» обратно на отдельные кнопки SEO / ASO / SMS / PUSH / IN APP.
      // Если в БД ещё лежат старые объединённые кнопки — удаляем их и кладём
      // пять раздельных + правильный _order. Идемпотентно: если уже разбиты,
      // ничего не делаем.
      const tc = data.screens.anketa_traffic_category;
      if (tc?.buttons && (tc.buttons.btn_seo_aso_sms || tc.buttons.btn_push_inapp)) {
        delete tc.buttons.btn_seo_aso_sms;
        delete tc.buttons.btn_push_inapp;
        tc.buttons.btn_seo = { label: 'SEO', action: 'callback:anketa_answer', targetScreen: 'anketa_sub_check' };
        tc.buttons.btn_aso = { label: 'ASO', action: 'callback:anketa_answer', targetScreen: 'anketa_sub_check' };
        tc.buttons.btn_sms = { label: 'SMS', action: 'callback:anketa_answer', targetScreen: 'anketa_sub_check' };
        tc.buttons.btn_push = { label: 'PUSH', action: 'callback:anketa_answer', targetScreen: 'anketa_sub_check' };
        tc.buttons.btn_inapp = { label: 'IN APP', action: 'callback:anketa_answer', targetScreen: 'anketa_sub_check' };
        tc.buttons._order = [
          'btn_cpa_net', 'btn_seo', 'btn_aso', 'btn_sms', 'btn_cpa',
          'btn_push', 'btn_inapp', 'btn_dsp', 'btn_social', 'btn_influence', 'btn_other_traffic',
        ];
