...
// ─── EVENT ANKETA QUESTIONS ─────────────────────────────────────────────────
// Table: event_questions(id, question_text, question_type, options JSON, `order`, is_active)

router.get('/event-questions', async (req, res, next) => {
  try {
    const [rows] = await dbPool.query(
      "SELECT id, question_text, question_type, options, `order`, is_active FROM event_questions ORDER BY `order` ASC, id ASC"
    );
    const list = rows.map(r => ({
      ...r,
      options: typeof r.options === 'string' ? JSON.parse(r.options || 'null') : r.options,
...
router.post('/event-questions', async (req, res, next) => {
  try {
    const { question_text, question_type = 'text', options = null, order = 0 } = req.body || {};
    if (!question_text || !question_text.trim()) return res.status(400).json({ error: 'question_text required' });
    const type = question_type === 'choice' ? 'choice' : 'text';
    const opts = type === 'choice' && Array.isArray(options) ? JSON.stringify(options) : null;
    const [result] = await dbPool.query(
      "INSERT INTO event_questions (question_text, question_type, options, `order`, is_active) VALUES (?, ?, ?, ?, 1)",
      [question_text.trim(), type, opts, Number(order) || 0]
    );
    res.status(201).json({ id: result.insertId });
  } catch (err) { next(err); }
});

router.put('/event-questions/:id', async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const { question_text, question_type, options, order, is_active } = req.body || {};
    const sets = [];
    const params = [];
    if (typeof question_text === 'string') { sets.push('question_text = ?'); params.push(question_text); }
    if (question_type) {
      const type = question_type === 'choice' ? 'choice' : 'text';
      sets.push('question_type = ?'); params.push(type);
    }
    if (options !== undefined) {
      sets.push('options = ?');
      params.push(Array.isArray(options) && options.length ? JSON.stringify(options) : null);
    }
    if (order !== undefined) { sets.push('`order` = ?'); params.push(Number(order) || 0); }
    if (is_active !== undefined) { sets.push('is_active = ?'); params.push(is_active ? 1 : 0); }
    if (!sets.length) return res.json({ ok: true });
    params.push(id);
    await dbPool.query(`UPDATE event_questions SET ${sets.join(', ')} WHERE id = ?`, params);
    res.json({ ok: true });
  } catch (err) { next(err); }
});

router.delete('/event-questions/:id', async (req, res, next) => {
  try {
...
router.put('/event-questions-order/reorder', async (req, res, next) => {
  try {
    const ids = Array.isArray(req.body?.ids) ? req.body.ids : [];
    for (let i = 0; i < ids.length; i++) {
      await dbPool.query('UPDATE event_questions SET `order` = ? WHERE id = ?', [i, Number(ids[i])]);
    }
    res.json({ ok: true });
  } catch (err) { next(err); }
});
